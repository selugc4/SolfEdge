...
El desarrollo de la aplicación se ha llevado a cabo siguiendo un proceso iterativo \cite{Pressman2010} compuesto por diferentes fases, con el objetivo de diseñar una solución que respondiera a las necesidades reales de la enseñanza del lenguaje musical y garantizara una implementación sólida y mantenible.
...
El aprendizaje del lenguaje musical presenta una serie de características que lo diferencian de otras disciplinas académicas. No se limita a la adquisición de conocimientos teóricos, sino que combina el estudio conceptual con el desarrollo de habilidades prácticas relacionadas con la lectura, la interpretación y la percepción auditiva \cite{UNIR_PedagogiaMusical}.
...
Con el objetivo de desarrollar una aplicación web moderna, mantenible y fácilmente escalable, se ha optado por una arquitectura basada en un modelo cliente-servidor \cite{Tanenbaum2007} con una separación clara entre frontend y backend, apoyada en servicios REST \cite{PostmanREST}. Esta decisión responde tanto a criterios técnicos como a condicionantes generales del proyecto, tales como la viabilidad en un contexto académico, el uso de tecnologías gratuitas o con planes de uso sin coste durante el desarrollo, y la necesidad de facilitar la evolución futura del sistema.
...
Además, este enfoque arquitectónico facilita la aplicación de buenas prácticas de desarrollo y la incorporación de estándares de accesibilidad (como WCAG \cite{W3C}), al permitir una estructuración clara de las vistas, la lógica de negocio y la gestión de estados.
...
En materia de seguridad, se integra \textbf{JSON Web Tokens (JWT)} \cite{JWTIntro} para la autenticación y autorización de usuarios. Este mecanismo garantiza la identidad de los usuarios mediante tokens firmados, evitando accesos no autorizados y alineándose con buenas prácticas de seguridad ampliamente aceptadas \cite{OWASP}.
...
\end{sidewaysfigure}
\clearpage
\chapter {Análisis, definición y arquitectura de la solución}
...
Las preguntas y respuestas realizadas en las entrevistas de ambos profesores han sido las siguientes las que aparecen en la Figura \ref{fig:entrevistasProfe}
...
La encuesta realizada para los alumnos ha sido la misma para ambos centros, ya que no se ha considerado que personas de una edad tan temprana puedan apreciar diferencias en función del centro. Las preguntas realizadas aparecen en la próxima Figura \ref{fig:encuestaAlumnos}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.45\linewidth]{Figuras/encuestaA.png}
    \caption{Encuesta al alumnado}
    \label{fig:encuestaAlumnos}
\end{figure}

\newpage

Se exponen en las gráficas de la Figura \ref{fig:graficaAlumnos} las respuestas a las preguntas 1, 3 y 6 en primer lugar, se debe tener en cuenta que el total de respuestas de cada pregunta de ambas gráficas varía un poco debido a que, en algunos casos, dicha respuesta estaba en blanco o no era adecuada.
...
\begin{figure}[h]
    ...
    \label{fig:graficaAlumnos}
\end{figure}

Las respuestas de la pregunta 4 también han sido reflejadas en las gráficas de la Figura \ref{fig:graficaAlumnos2}
...
Los motores de búsqueda utilizados han sido: Google Scholar, Scopus, Association for Computing Machinery (ACM) Library y UVAdoc, este último, porque al hacer búsquedas previas en crudo para analizar los campos, en la universidad de Valladolid (a la cual pertenece el repositorio mencionado), ya se habían hecho algunos TFGs similares. Para el motor de búsqueda de Google se han analizado tan solo los primeros 50 resultados ya que, a partir de ahí, comenzaban a tener muy poca relevancia. Todo esto aparece esquematizado en el flujograma de la Figura \ref{fig:flujograma}
...
Se puede comprobar la documentación obtenida (ordenada de por fecha de publicación descendente) en la Figura \ref{fig:documentacion}.
...
\textbf{Actor} & ACT-3\\ \hline
\textbf{Requisitos funcionales} & RF-23 RF-24\\ \hline
\end{tabularx}
\caption{Caso de uso: Gestionar entregas}
\end{table}
\begin{table}[H]
...
\textbf{Secuencia de pasos} & 
\begin{enumerate}
  \item El profesor accede a administración y entra en la herramienta de creación de mensajes.
...
\begin{enumerate}
  ...
  \item El sistema muestra la lista de entregas realizadas por los alumnos (y su calificación, si la tuviera).
...
\begin{enumerate}
  ...
  \item El profesor, si lo desea, introduce una observación.
...
\begin{tabularx}{\textwidth}{
  ...
  |>{\centering\arraybackslash}X|}
\hline
\textbf{RF-25} & \textbf{Revisar calificaciones} \\ \hline

\textbf{Descripción} & Permitir al alumno revisar todas las calificaciones asignadas. \\ \hline

\textbf{Precondición} & El alumno debe estar autenticado, debe de haber un grupo seleccionado y existir una entrega asociada a la tarea seleccionada. \\ \hline

\textbf{Secuencia de pasos} &
\begin{enumerate}
  \item El alumno selecciona su perfil.
  \item El alumno selecciona si quiere ver calificaciones generales o continuas.
  \item El sistema muestra la lista de las calificaciones.
\end{enumerate} \\ \hline

\textbf{Postcondición} & El alumno visualiza correctamente las calificaciones. \\ \hline

\textbf{Excepciones} &
\begin{itemize}
  \item No existen calificaciones y/o continuas.
  \item No se ha elegido ningún grupo.
  \item Problemas de conexión a la base de datos.
\end{itemize} \\ \hline
\end{tabularx}
\caption{Requisito funcional: RF-25 Revisar calificaciones}
\label{tab:RF-25}
\end{table}
\begin{table}[H]
\centering
...
\textbf{Descripción} & Permitir al profesor revisar las calificaciones generales obtenidas previamente por un alumno en el curso. \\ \hline

\textbf{Precondición} & El profesor debe de estar autenticados, tener un grupo seleccionado y que el alumno tenga calificaciones generales registradas en el sistema. \\ \hline

\textbf{Secuencia de pasos} &
\begin{enumerate}
  \item El profesor accede a administración.
  \item Si el profesor es alumno, el sistema identifica automáticamente al alumno autenticado.
  \item Si el profesor es profesor, el sistema requiere de la selección del alumno desde la herramienta de calificaciones generales.
  \item El sistema muestra las calificaciones generales registradas para el alumno.
  \item El profesor revisa el historial de calificaciones.
\end{enumerate} \\ \hline

\textbf{Postcondición} & El profesor visualiza correctamente las calificaciones generales del alumno. \\ \hline
...
\textbf{Secuencia de pasos} &
\begin{enumerate}
  \item El profesor selecciona el perfil.
...
1. Validar los datos de entrada:
   ...
   - Tamaño correcto del string de observaciones
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{Figuras/frontend_login_mobile.png}
    \includegraphics[width=1\textwidth]{Figuras/frontend_login_desktop.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.15\textwidth]{Figuras/frontend_menu_lateral.png}
    \includegraphics[width=0.8\textwidth]{Figuras/frontend_barra_pestañas.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Figuras/frontend_calificaciones_ioncard.png}
...
Como ejemplo representativo se muestra el conjunto de pruebas desarrollado para el componente \texttt{ProfessorAdminpage}, encargado de gestionar distintas funcionalidades relacionadas con la interacción del usuario.

\begin{Verbatim}[frame=single, fontsize=\small]
Pruebas del componente ProfessorAdminPage

1. Verificar la creación del componente.

2. Verificar la inicialización del usuario autenticado.

3. Verificar la finalización del estado de carga.

4. Verificar la apertura del modal de mensajes.

5. Verificar la apertura del modal de calificación
   general cuando existe un grupo seleccionado.

6. Verificar la apertura del modal de gestión
   de grupo cuando existe un grupo seleccionado.

7. Verificar la interacción con los servicios
   simulados y el controlador de modales.
\end{Verbatim}
...
Los requisitos implicados en este flujo son: RF-19 (Figura~\ref{tab:RF-19}), RF-25 (Figura \ref{tab:RF-25}, RF-28 (Figura~\ref{tab:RF-28}), RF-30 (Figura~\ref{tab:RF-30}) y RFT1 (Figura~\ref{tab:RFT-1}), tal y como se ha descrito en el capítulo de análisis de requisitos.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Figuras/mensaje_creacion.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figuras/resolver_cuestionario.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Figuras/revision_cuestionario.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{Figuras/crear_tarea.png}
    \includegraphics[width=0.6\textwidth]{Figuras/confirmacion_tarea.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figuras/mensaje_creacion_tarea.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{Figuras/entrega_tarea.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{Figuras/mensaje_entrega.png}
...
El sistema registra la información introducida y ya se puede comprobar las calificaciones desde la tarea.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{Figuras/calificar_tarea.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Figuras/calificaciones_tarea.png}
...
En esta pantalla la aplicación muestra la calificación obtenida durante el proceso de corrección, permitiendo al alumno conocer el resultado alcanzado durante la evaluación.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Figuras/revision_tarea.png}
...
\section{Flujo de trabajo de calificaciones generales}
...
Los requisitos implicados en este flujo son RF-25 (Figura \ref{tab:RF-25}), RF-26 (Figura ~\ref{tab:RF-26}), RF-27 (Figura ~\ref{tab:RF-27}), RF-28 (Figura ~\ref{tab:RF-28}) y RFT-1 (Figura ~\ref{tab:RFT-1}), tal y como se ha descrito en el capítulo de análisis de requisitos.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{Figuras/calificaciones_profesor.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Figuras/calificaciones_alumno.png}
...
En este apartado se incluyen capturas adicionales de la aplicación en funcionamiento que complementan las mostradas anteriormente. Estas imágenes permiten apreciar otras vistas y flujos relevantes del sistema, reforzando la comprensión global de la aplicación desarrollada. En este caso, en la Figura \ref{fig:vista_rama} se puede observar la vista de la rama de ritmo (con metrónomo incorporado para el alumno), en la Figura \ref{fig:vista_administracion} la vista del profesor y del administrador de la propia administración y en la Figura \ref{fig:mensajeria} la vista de mensajería.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=0.8\textwidth]{Figuras/frontend_rama.png}
        \caption{Vista de profesor con libro y tareas}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=0.6\textwidth]{Figuras/frontend_rama_vacia.png}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{Figuras/frontend_administracion.png}
        \caption{Administración de administrador}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{1\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{Figuras/frontend_administracion_profesor.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Figuras/frontend_mensajes.png}
    \caption{Vista de mensajería}
    \label{fig:mensajeria}
\end{figure}

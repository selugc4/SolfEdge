...
Generación de una pista para preguntas de teoría

1. Verificar que la clave de acceso a la API
   está configurada.

2. Obtener y formatear las opciones de respuesta.

3. Construir el mensaje con la pregunta,
   las opciones y las instrucciones para
   generar una pista.

4. Enviar la petición al modelo de IA.

5. Obtener la pista generada.

6. Verificar que la respuesta no está vacía.

7. Limpiar el texto y devolver la pista.
\end{Verbatim}
...
/cuestionarios/:id/preguntas/:index/pista

1. Verificar que devuelve una pista
   correctamente cuando la petición es válida.

2. Verificar que devuelve un error 400
   cuando el índice de la pregunta es inválido.

3. Verificar que devuelve un error 503
   cuando el servicio de pistas se encuentra
   en mantenimiento.
\end{Verbatim}
...
1. Detectar si el tamaño de la pantalla
   corresponde a un dispositivo móvil.

2. Verificar que existe un archivo PDF.

3. Convertir el archivo PDF al formato Base64.

4. Guardar temporalmente el archivo
   en el dispositivo.

5. Abrir el archivo PDF con la aplicación
   predeterminada.

6. Mostrar un mensaje de error si el PDF
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.2\textwidth]{Figuras/menus.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{Figuras/Ritmo_pc.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1\textwidth]{Figuras/cuestionario_pc.png}
  \caption{Vista de un cuestionario en la versión web.}
  \label{fig:cuestionario_pc}
\end{figure}
\begin{figure}[H]
  \centering

  \includegraphics[width=0.3\textwidth]{Figuras/cuestionario.png}
  \caption{Vista de un cuestionario en la versión en Android.}
...
Se presenta un pseudocódigo que resume el comportamiento validado por los distintos casos de prueba implementados.
...
\newpage
\begin{Verbatim}[frame=single, fontsize=\small]
Despliegue continuo del frontend y backend

1. Verificar que la ejecución se realiza
   desde la rama principal.

2. Esperar a que finalice la fase de
   construcción del proyecto.

3. Obtener el código fuente del repositorio.

4. Autenticarse en Azure.

5. Descargar los artefactos generados
   (solo para el frontend).

6. Construir la imagen Docker de la
   aplicación correspondiente.

7. Autenticarse en el registro de
   contenedores de Azure.

8. Publicar la imagen Docker en el
   registro de contenedores.

9. Configurar y desplegar la nueva imagen

...
\begin{itemize}
    ...
    \item \textbf{Creación del Azure Container Registry (ACR):} Se crea un registro privado de contenedores donde se almacenarán las imágenes Docker del frontend y backend de SolfEdge. Este registro permite la gestión segura, versionada y centralizada de las imágenes, facilitando su despliegue automatizado.
...
\begin{enumerate}
    ...
    \item \textbf{Configuración del proveedor de correo}: Se empleará un proveedor transaccional (por ejemplo, Mailjet \cite{firebase_storage_docs} u otro equivalente), centralizando el envío de correos desde un servicio controlado y monitorizable.
...
El cronograma propuesto se adapta a un centro educativo de tamaño medio y contempla una implantación progresiva en tres fases (piloto, parcial y completa), alineada con los tiempos ya establecidos en el propio plan: un piloto de cuatro a seis semanas, un despliegue parcial de dos a tres meses y una transición final hacia despliegue completo. Además, integra tareas transversales (seguridad, correo y formación) para garantizar que el crecimiento de usuarios se realice sobre una base estable y controlada.

La Figura~\ref{fig:cronograma_implantacion} muestra el cronograma completo con la secuencia temporal de tareas y los hitos principales: preparación del entorno productivo, arranque del piloto, emisión del informe de validación, ampliación a despliegue parcial, consolidación operativa y paso a despliegue completo. Se incluyen también revisiones periódicas trimestrales asociadas al seguimiento de seguridad y dependencias externas, en coherencia con la Sección~\ref{sec:seguridad_implantacion}.
...
\begin{figure}[H]
  ...
  \caption{Demostración de la aplicación SolfEdge en un entorno real de aula}
  \label{fig:demo_aula}
\end{figure}

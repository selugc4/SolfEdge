...
En este punto, el objetivo de este complemento no es replantear la solución desde cero, sino evolucionarla de manera controlada. Se mantiene la arquitectura general, la filosofía de funcionamiento y la mayor parte de requisitos ya definidos en el documento principal, lo que se incorpora ahora se centra en:
...
Los objetivos funcionales descritos en el versión inicial se mantienen, ya que la esencia del sistema no cambia: facilitar la enseñanza del lenguaje musical con herramientas específicas de gestión y aprendizaje. No obstante, este complemento añade objetivos de evolución funcional centrados en la operatividad real y en la mejora de la experiencia del alumno:
...
Capacitor surge como la evolución natural de este enfoque, manteniendo la reutilización del código web, pero adaptándolo a las necesidades actuales del desarrollo móvil. La diferencia entre ambos modelos puede resumirse claramente en los siguientes puntos:
...
Las operaciones de revisión (listados y consultas) reutilizan el mismo patrón del trabajo previo: filtrado por rol, respuesta en JSON y acceso a través de controladores. En cambio, las eliminaciones se han ampliado para contemplar el efecto real en otras colecciones (por ejemplo: al eliminar un grupo se eliminan/limpian tareas, cuestionarios, entregas y calificaciones asociadas, y se elimina la relación en los alumnos).
...
// partes nativas del sistema (barra de estado o botonera inferior del sistema nativo).
...
\begin{Verbatim}[frame=single, fontsize=\small, xleftmargin=10pt, xrightmargin=10pt]
//Se comprueba al inicio si es pantalla reducida (sistemas Android o iOS de tablet y móvil).
...
Todo esto lo maneja Android Studio para cada aplicación añadiendo unos XML que contienen las referencias a los distintos logos y estilos usados, en concreto, AndroidManifest (donde se declara el nombre, icono y permisos) y splash.xml (pantalla de carga)
...
La migración de los servicios web hacia un entorno productivo se apoyará en la plataforma \textbf{Microsoft Azure}, aprovechando su ecosistema cloud maduro, su integración con herramientas de desarrollo modernas y su capacidad para proporcionar una infraestructura altamente escalable, resiliente y monitorizable.

La arquitectura definida en el apartado~\ref{sec:tecnologias_ext} facilita este proceso, ya que el sistema presenta una clara separación entre frontend, backend y base de datos, permitiendo una transición progresiva sin necesidad de modificar la lógica de la aplicación. En este contexto, Azure App Service se utilizará como plataforma unificada tanto para el despliegue del backend como del frontend, garantizando una gestión homogénea de los recursos y un control centralizado del entorno productivo.

\subsubsection{Azure App Service como plataforma de ejecución}

Azure App Service proporciona un entorno gestionado para la ejecución de aplicaciones web, APIs REST y servicios backend, abstraendo la complejidad asociada a la gestión directa de servidores, redes, balanceadores de carga y sistemas operativos. Este modelo permite centrar los esfuerzos en el desarrollo funcional, delegando en la plataforma cloud la responsabilidad del escalado, la disponibilidad y la seguridad perimetral.
...
\begin{itemize}
    ...
    \item \textbf{Integración nativa con CI/CD}: Azure App Service se integra de forma directa con GitHub Actions, facilitando la automatización completa del flujo de despliegue, tal y como se describe en el capítulo~\ref{chap:despliegue}, de esta forma, todas las operaciones de la migración quedarán integradas de forma automática con tan solo unas líneas en el archivo .YML que lee GitHub Actions.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Servicios auxiliares}: almacenamiento de variables de entorno, gestión segura de secretos y configuración centralizada mediante Azure Key Vault.
\end{itemize}
...
El objetivo principal de esta migración es que el cambio afecte únicamente al flujo de despliegue, sin introducir modificaciones en la lógica del sistema. La estructura modular del proyecto y el uso de contenedores de ejecución homogéneos permiten esta transición sin reescritura del código.

A continuación se muestra un pipeline orientativo que ilustra la construcción, publicación de imágenes Docker en un registro privado (Azure Container Registry, ACR) y despliegue automatizado en \textit{Azure App Service for Containers}. Este flujo se apoya en los pasos ya consolidados de construcción y testeo automáticos descritos en el despliegue del proyecto original, y únicamente adapta la fase final de publicación para ejecutarse en Azure.
...
\begin{Verbatim}[frame=single, fontsize=\small]
  push-and-deploy-frontend:
    ...
    steps:
      ...
      # Autenticación contra Azure mediante credenciales seguras
      - name: Login to Azure CLI
        uses: azure/login@v1
        with:
          creds: ${{ secrets.AZURE_CREDENTIALS }}

      # Descarga de los artefactos generados previamente
      ...
      # Construcción de la imagen Docker del frontend y subida
      # al registro privado Azure Container Registry (ACR)
      - name: Build and push frontend Docker image to ACR
        run: |
          IMAGE_NAME=repodocker.azurecr.io/twm-frontend:\
${{ github.run_number }}
...
--password-stdin
      ...
      # Despliegue del contenedor del frontend en Azure Web App
...
https://repodocker.azurecr.io \
            --docker-registry-server-user \
${{ secrets.AZURE_USERNAME }} \
            --name TWM-FRONTEND \
  ...
  push-and-deploy-backend:
    ...
    steps:
      # Descarga del código fuente del backend
      - name: Checkout code
        uses: actions/checkout@v3
      ...
      # Construcción de la imagen Docker del backend y subida a ACR
      - name: Build and push backend Docker image to ACR
        run: |
          IMAGE_NAME=repodocker.azurecr.io/twm-backend:\
${{ github.run_number }}
...
--password-stdin
      ...
      # Despliegue del backend en Azure Web App usando contenedores
...
${{ secrets.AZURE_USERNAME }} \
            ...
            --resource-group cnsa-2024-rg
\end{Verbatim}
Este pipeline automatiza de forma completa la publicación del sistema en el entorno productivo, reduciendo el riesgo de errores manuales y garantizando trazabilidad de versiones mediante etiquetas ligadas a cada ejecución (\texttt{run\_number}). Además, al basarse en contenedores, permite reproducibilidad del entorno de ejecución y facilita futuras ampliaciones del proceso.

% \begin{figure}[H]
%   \centering
%   \includegraphics[height=2cm]{Figuras/logos/azure.png}
%   \caption{Logo Microsoft Azure}
%   \label{fig:logo_azure}
% \end{figure}

\subsection{Escalado de la base de datos con MongoDB Atlas}
\label{sec:escalado_mongodb}

La base de datos constituye un componente crítico dentro del proceso de implantación, al centralizar toda la información académica, administrativa y de seguimiento del alumnado. Por este motivo, el plan contempla una estrategia de escalado progresivo utilizando las capacidades avanzadas de \textbf{MongoDB Atlas}, descritas previamente en el apartado~\ref{sec:tecnologias_ext}.
...
Por este motivo, el plan de implantación contempla una migración progresiva hacia un sistema de almacenamiento externo especializado, como \textbf{Firebase Storage} o \textbf{Azure Blob Storage}, permitiendo desacoplar la gestión de ficheros del núcleo transaccional del sistema.
...
// Recuperación
const buffer = Buffer.from(libro.contenido, 'base64');
res.contentType(libro.tipo).send(buffer);
\end{Verbatim}

\textbf{Modelo propuesto (almacenamiento externo con referencia):}

\begin{Verbatim}[frame=single, fontsize=\small]
// Subida del archivo al servicio externo
const fileUrl = await storageService.upload(file);

// Persistencia de metadatos y referencia
await Libro.create({
  nombre: 'libro.pdf',
  url: fileUrl,
  tipo: 'application/pdf'
...
\begin{enumerate}
    ...
    \item \textbf{Configuración del proveedor de correo}: se empleará un proveedor transaccional (por ejemplo, Mailjet u otro equivalente), centralizando el envío de correos desde un servicio controlado y monitorizable.
...
\begin{itemize}
    \item Manual de usuario para profesorado.
    \item Guía rápida para alumnado.
    \item Documento de preguntas frecuentes (FAQ) y guía básica de resolución de incidencias.
\end{itemize}

Estos materiales se desarrollarían antes del despliegue parcial del sistema, sirviendo como soporte permanente durante las primeras fases de adopción y facilitando el autoaprendizaje.
...
\subsection{Evidencia visual de la demostración}

A continuación se muestran algunas capturas realizadas durante la sesión de demostración, en las que puede observarse el uso real de la aplicación por parte del alumnado y el profesorado.

% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\textwidth]{Figuras/implantacion_real/demostracion_aula_1.jpg}
%   \caption{Demostración de la aplicación en aula real con alumnado.}
%   \label{fig:demo_aula_1}
% \end{figure}

% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\textwidth]{Figuras/implantacion_real/demostracion_aula_2.jpg}
%   \caption{Interacción directa del alumnado con la aplicación durante la sesión.}
%   \label{fig:demo_aula_2}
% \end{figure}

% \begin{figure}[H]
%   \centering
%   \includegraphics[width=0.9\textwidth]{Figuras/implantacion_real/demostracion_profesora.jpg}
%   \caption{Uso del panel docente durante la demostración práctica.}
%   \label{fig:demo_profesora}
% \end{figure}

Estas evidencias visuales refuerzan el carácter aplicado del proyecto y demuestran su viabilidad real en contextos educativos auténticos.

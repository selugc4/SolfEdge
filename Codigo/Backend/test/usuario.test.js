...
// Mockear authMiddleware para que siempre pase (por defecto admin-user)
...
describe('Usuario API', () => {
  ...
  describe('DELETE /usuarios/:id', () => {
    ...
    beforeEach(() => {
      // Alumno: borrado de datos asociados
      ...
      // Si tu controlador todavía llama a updateMany en otros caminos, déjalo también:
      Usuario.updateMany = Usuario.updateMany || jest.fn();
      Usuario.updateMany.mockResolvedValue({});

      // Asegura que existan deleteMany de profesor (si tu código las llama)
    ...
    it('should delete a student and associated data successfully by creator', async () => {
      ...
      Usuario.findById.mockResolvedValueOnce({
        _id: alumnoId,
        role: 'alumno',
      ...
      expect(Mensaje.deleteMany).toHaveBeenCalledWith({
        $or: [{ emisor: alumnoId }, { receptores: alumnoId }],
      });
    ...
    it('should delete a professor and all associated data successfully by admin', async () => {
      ...
      Usuario.findById.mockResolvedValueOnce({ _id: profesorId, role: 'profesor' }); // a eliminar
      Usuario.findById.mockResolvedValueOnce({ _id: adminId, role: 'administrador' }); // admin logeado

      // Cascadas del profesor
      ...
      expect(Mensaje.deleteMany).toHaveBeenCalledWith({
        $or: [{ emisor: profesorId }, { receptores: profesorId }],
      });

...
describe('CuestionarioEditPage', () => {
  ...
  const mockGrupo = { _id: 'grupo1', nombre: 'Test Grupo', alumnos: [{ _id: 'alumno1' }] };
  ...
  const cuestionarioServiceMock = {
    ...
    crearCuestionario: jasmine.createSpy('crearCuestionario').and.returnValue(of(mockCuestionario)),
    updateCuestionario: jasmine.createSpy('updateCuestionario').and.returnValue(of(mockCuestionario)),
    uploadAudioRecurso: jasmine.createSpy('uploadAudioRecurso').and.returnValue(of({}))
  };
  ...
  const configureTestBed = (isEditMode: boolean) => {
    ...
    TestBed.configureTestingModule({
      imports: [CuestionarioEditPage],
      providers: [
        { provide: ActivatedRoute, useValue: { snapshot: { paramMap: { get: paramMap.get.bind(paramMap) }, queryParamMap: { get: queryParamMap.get.bind(queryParamMap) } } } },
  ...
  describe('Create Mode', () => {
    ...
    it('should call crearCuestionario on submit', () => {
      fixture.detectChanges();
      component.form.patchValue({ nombre: 'Nuevo Cuestionario', alumnos: ['alumno1'] });
      component.preguntas.at(0).patchValue({ texto: 'q1', respuestaCorrecta: '0' });
      component.getPosiblesRespuestas(0).at(0).patchValue({ texto: 'a' });
      component.getPosiblesRespuestas(0).at(1).patchValue({ texto: 'b' });

      cuestionarioServiceMock.crearCuestionario.calls.reset(); // Reset spy calls antes de usar
      component.onSubmit();
  ...
  describe('Edit Mode', () => {
    ...
    it('should call updateCuestionario on submit', async () => {
      ...
      component.form.patchValue({ nombre: 'Cuestionario Actualizado', alumnos: ['alumno1'] });
      ...
      if (respuestas.length < 2) {
        respuestas.push(new FormGroup({ texto: new FormControl('Respuesta correcta', Validators.required) }));
        respuestas.push(new FormGroup({ texto: new FormControl('Respuesta incorrecta', Validators.required) }));
      } else {
        respuestas.at(0).patchValue({ texto: 'Respuesta correcta' });
        respuestas.at(1).patchValue({ texto: 'Respuesta incorrecta' });
      }

      component.form.markAsDirty();
      component.form.updateValueAndValidity();

      cuestionarioServiceMock.updateCuestionario.calls.reset(); // Reset spy calls antes de usar
      component.onSubmit();
      await fixture.whenStable();
